import { firebaseConfig, USER_PROFILES, DEFAULT_SEASON_ID } from "./firebase-config.js";
...
async function revealAndCalculate(matchId, actualData) {
  ...
  const updatedMatch = {
    ...match,
    actualHome: actualData.actualHome,
    actualAway: actualData.actualAway,
    actualScorers: parseScorers(actualData.actualScorers),
    revealed: true
  };
  ...
  await updateDoc(doc(db, "matches", matchId), {
    actualHome: updatedMatch.actualHome,
    actualAway: updatedMatch.actualAway,
    actualScorers: updatedMatch.actualScorers,
    revealed: true,
    calculated: true,
    summary,
...
function renderMatchList(containerId, gameType) {
  ...
  openMatches.forEach(match => {
    ...
    saved.textContent = myPred ? "Your prediction is saved. You can still edit until reveal." : "No prediction saved yet.";
    ...
    if (myPred && !match.revealed) {
      ...
      const scorers = parseScorers(myPred.scorers);
      node.querySelector('[data-own="scorer0"]').value = scorers[0] || "";
      node.querySelector('[data-own="scorer1"]').value = scorers[1] || "";
      node.querySelector('[data-own="scorer2"]').value = scorers[2] || "";
    }
    ...
    node.querySelector('[data-actual="actualScorers"]').value = parseScorers(match.actualScorers).join(", ");
    ...
    node.querySelector(".reveal-btn").addEventListener("click", async () => {
      await revealAndCalculate(match.id, {
        ...
        actualScorers: node.querySelector('[data-actual="actualScorers"]').value
      });
    });
...
async function boot() {
  ...
  onAuthStateChanged(auth, async user => {
    ...
    currentProfile = USER_PROFILES[email];

    if (!currentProfile) {
      showApp(false);
      setCloudStatus("Signed in, but not allowed", "offline");
      alert("This email is not listed in USER_PROFILES inside firebase-config.js.");
      return;
    }

    showApp(true);
    setCloudStatus(`Signed in as ${currentProfile.name}`, "online");
